...
const SUBMISSION_FIELDS: FormField[] = [
  {
    label: "Product URL",
    value: "https://jobfiltr.app",
    spec: "Direct link, no UTM or shortened URLs",
    copyable: true,
  },
  {
    label: "Product Name",
    value: "JobFiltr",
    spec: "Name only — no marketing claims or emojis",
    copyable: true,
  },
  {
    label: "Tagline",
    value: "Stop wasting time on ghost jobs, scams, and spam listings",
    spec: "60 characters max (this is 57)",
    copyable: true,
  },
  {
    label: "Description",
    value:
      "JobFiltr is a Chrome extension that filters and analyzes job postings on LinkedIn and Indeed in real time. It uses AI to detect ghost jobs (50+ signals), flags scams and spam, hides staffing agencies, shows job age badges, and lets you filter by keywords, salary, and more. Zero tracking, zero data collection. Your job search, upgraded.",
    spec: "500 characters max (this is 333)",
    copyable: true,
  },
  {
    label: "Launch Tags",
    value: "Chrome Extensions, Productivity, Job Search",
    spec: "Up to 3 category tags",
    copyable: true,
  },
  {
    label: "Pricing",
    value: "Free (or 'Paid with free plan' if pricing exists by launch)",
    copyable: false,
  },
];

const MAKER_COMMENT = `Hey Product Hunt! I'm [Name], the maker behind JobFiltr.
...
const CHECKLIST_SECTIONS: ChecklistSection[] = [
    ...
    items: [
      {
        ...
        detail: "240x240px min (600x600 recommended for Retina). JPG/PNG/GIF, max 3MB.",
      },
      {
        id: "asset-gallery",
        label: "Gallery Images (minimum 2)",
        detail: "1270x760px each. One idea per image — headline + single benefit per slide.",
      },
      {
        id: "asset-video",
        label: "Video (optional but recommended)",
        detail: "YouTube full URL (not private). 45-60s: Hook (8s) -> Problem (10s) -> Demo (20s) -> CTA.",
      },
      {
        id: "asset-comment",
        label: "Maker Comment prepared",
        detail: "3-5 paragraphs. Cover: origin story, problem, unique solution, ask for feedback.",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      {
        id: "ld-midnight",
        label: "12:00-12:05 AM PST",
        detail: "Verify page live, post maker comment, first Twitter/X announcement.",
      },
      {
        id: "ld-early",
        label: "12:05 AM - 4:00 AM PST",
        detail: "Send first email blast (15-20% of list), respond to every PH comment, DM closest supporters.",
      },
      {
        id: "ld-europe",
        label: "4:00-6:00 AM PST",
        detail: "Message European supporters (their morning).",
      },
      {
        id: "ld-linkedin",
        label: "6:00 AM PST",
        detail: "LinkedIn post targeting European audience.",
      },
      {
        id: "ld-morning",
        label: "7:00-8:00 AM PST",
        detail: "Respond to overnight comments. Main email blast (American morning).",
      },
      {
        id: "ld-midmorning",
        label: "10:00 AM PST",
        detail: "Check ranking, share milestone if top 5.",
      },
      {
        id: "ld-noon",
        label: "10:30 AM - 12:00 PM PST",
        detail: "Next supporter batch, post on Reddit (if established presence), behind-the-scenes thread.",
      },
      {
        id: "ld-afternoon",
        label: "12:00-4:00 PM PST",
        detail: "Midday social update, share interesting metric or feedback quote.",
      },
      {
        id: "ld-evening",
        label: "4:00-8:00 PM PST",
        detail: "Follow-up email to non-openers (plain-text), social reminder.",
      },
      {
        id: "ld-night",
        label: "8:00-11:59 PM PST",
        detail: "Final transparent update about ranking. Screenshot final ranking at midnight.",
      },
    ],
  },
  {
                ...
                {SUBMISSION_FIELDS.map((field) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      {field.copyable && (
                        <button
                          onClick={() => copyText(`ph-${field.label}`, field.value)}
          ...
          <div>
            ...
            {!collapsed["maker-comment"] && (
              <div className="mt-3 relative">
                ...
                <button
                  onClick={() => copyText("ph-maker-comment", MAKER_COMMENT)}
              ...
              <div className="mt-3 rounded-lg bg-red-500/5 border border-red-500/20 p-4 space-y-1.5">
                {RULES.map((rule) => (
                  <div key={rule} className="flex items-start gap-2 text-sm">
                    ...
                    <span className={rule.startsWith("NEVER") ? "text-red-300" : "text-white/70"}>
                      {rule}
                    </span>
                  </div>
                ))}
