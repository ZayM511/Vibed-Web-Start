 ...
 * Chrome Extension Version (JavaScript)
 *
...
/**
 * Create a reported company entry
 * @param {string} name - Display name
 * @param {string} category - spam | ghost | scam
 * @param {string[]} aliases - Alternative names
 * @returns {Object} Company entry
 */
function createEntry(name, category = 'ghost', aliases = []) {
  return {
    name,
 ...
 */
function buildReportedCompanyMap() {
  const map = new Map();
...
const REPORTED_COMPANY_MAP = buildReportedCompanyMap();

/**
 * Reported Company Detector Class
 */
class ReportedCompanyDetector {
  constructor() {
   ...
   * @param {Object} job - Job data with company field
  ...
  /**
   * Quick check if a company name might match
   */
  quickCheck(companyName) {
...
const reportedCompanyDetector = new ReportedCompanyDetector();

// Export for ES modules
export {
  ReportedCompanyDetector,
  reportedCompanyDetector,
  REPORTED_COMPANIES,
